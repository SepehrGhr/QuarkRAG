...
class KafkaProducerManager:
    ...
    async def start(self):
        ...
        logger.info("Query service Kafka producer started")

    async def stop(self):
        if self.producer:
            await self.producer.stop()
            logger.info("Query service Kafka producer stopped")
    ...
    async def send_message(self, topic: str, key: str, value: dict):
        ...
        logger.debug("Message sent from query service", topic=topic, key=key)
